...
<tr>
    ...
    <tr>
     ...
     <g:each in="${admin.DictionaryItem.findAllByDictionary_id(admin.Dictionary.findByCode("CHAD33F"))}" status="i" var="educationListInstance">
                ...
                def noHoldMember=0
                  if(end_date&&from_date){
                   noHoldMember=materialize.view.ViewHealthEducation.executeQuery("select sum(h.member_no) from ViewHealthEducation h where category=:category and education_type=:education_type and  arrival_time between '"+from_date+"' and '"+end_date+"'",[category:categorysListInstance,education_type:educationListInstance])[0]

                  }else{
                      noHoldMember=materialize.view.ViewHealthEducation.executeQuery("select sum(h.member_no) from ViewHealthEducation h where category=:category and education_type=:education_type ",[category:categorysListInstance,education_type:educationListInstance])[0]
        ...
        <%
                    def noHoldMember=0
                if(end_date&&from_date){
                 noHoldMember=materialize.view.ViewHealthEducation.executeQuery("select sum(j.member_no) from ViewHealthEducation j where category=:category and education_type=:education_type and  arrival_time between '"+from_date+"' and '"+end_date+"'",[category:categorysListInstance,education_type:educationListInstance])[0]

                }else{
                    noHoldMember=materialize.view.ViewHealthEducation.executeQuery("select sum(j.member_no) from ViewHealthEducation j where category=:category and education_type=:education_type ",[category:categorysListInstance,education_type:educationListInstance])[0]
