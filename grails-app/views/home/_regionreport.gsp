    ...
    <td class="info"><span>${chadNo}</span></td>
        ...
        <td>${houseHoldNo}</td>
         ...
         def houseHoldMember=0
             if(end_date&&from_date){
               houseHoldMember=chaid.Household.executeQuery("select sum(total_members),sum(male_no),sum(female_no) from Household where district_id=:district and deleted=false and created_at between '"+from_date+"' and '"+end_date+"'",[district:districtListInstance] )
             }else{
             houseHoldMember=chaid.Household.executeQuery("select sum(total_members),sum(male_no),sum(female_no) from Household where district_id=:district and deleted=false",[district:districtListInstance] )
