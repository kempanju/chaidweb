                ...
                <td>
                    <table class="table" border="1">
                            <%
                                ...
                                if (end_date && from_date) {
                                    houseHoldNo1 = MkChaid.executeQuery("select household.id from MkChaid where distric.region_id=:region and deleted=false and arrival_time between '" + from_date + "' and '" + end_date + "'  and visit_type=:visitTypeInstance and repeated=false", [region: regionListInstance, visitTypeInstance: visitTypeInstance]).size()

                                } else {
                                    houseHoldNo1 = MkChaid.executeQuery("select household.id from MkChaid where distric.region_id=:region and deleted=false  and visit_type=:visitTypeInstance and repeated=false", [region: regionListInstance, visitTypeInstance: visitTypeInstance]).size()

                                }

                                def houseHoldNo2 = 0;
                                if (end_date && from_date) {
                                    houseHoldNo2 = MkChaid.executeQuery("select household.id from MkChaid where distric.region_id=:region and deleted=false and arrival_time between '" + from_date + "' and '" + end_date + "'  and visit_type=:visitTypeInstance and repeated=true", [region: regionListInstance, visitTypeInstance: visitTypeInstance]).size()

                                } else {
                                    houseHoldNo2 = MkChaid.executeQuery("select household.id from MkChaid where distric.region_id=:region and deleted=false  and visit_type=:visitTypeInstance and repeated=true", [region: regionListInstance, visitTypeInstance: visitTypeInstance]).size()
                ...
                <td colspan="3">
                    <table class="table" border="1">
                        <%
                            ...
                            if (end_date && from_date) {
                                houseHoldMember = MkChaid.executeQuery("select sum(household.total_members),sum(household.male_no),sum(household.female_no) from MkChaid  where   deleted=false and created_at between '" + from_date + "' and '" + end_date + "' and distric.region_id=:region and visit_type=:visitTypeInstance and repeated=false", [region: regionListInstance, visitTypeInstance: visitTypeInstance])

                                // repeatHouseMember = houseHoldNoMember - visitedNewHouseHoldMember;


                            } else {
                                houseHoldMember = MkChaid.executeQuery("select sum(household.total_members),sum(household.male_no),sum(household.female_no) from MkChaid  where  deleted=false and visit_type=:visitTypeInstance and repeated=false and distric.region_id=:region", [visitTypeInstance: visitTypeInstance, region: regionListInstance])
                            ...
                            if (end_date && from_date) {
                                houseHoldMember1 = MkChaid.executeQuery("select sum(household.total_members),sum(household.male_no),sum(household.female_no) from MkChaid  where   deleted=true and created_at between '" + from_date + "' and '" + end_date + "' and distric.region_id=:region and visit_type=:visitTypeInstance and repeated=true", [region: regionListInstance, visitTypeInstance: visitTypeInstance])

                                // repeatHouseMember = houseHoldNoMember - visitedNewHouseHoldMember;


                            } else {
                                houseHoldMember1 = MkChaid.executeQuery("select sum(household.total_members),sum(household.male_no),sum(household.female_no) from MkChaid  where  deleted=false and visit_type=:visitTypeInstance and repeated=true and distric.region_id=:region", [visitTypeInstance: visitTypeInstance, region: regionListInstance])
                ...
                <td>
                    <table class="table" border="1">
                        <%
                            ...
                            if (end_date && from_date) {
                                womenAgeBetweenNo = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and arrival_time between '" + from_date + "' and '" + end_date + "' and chaid.visit_type=:visitTypeInstance and chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17G"), region: regionListInstance, visitTypeInstance: visitTypeInstance])

                            } else {
                                womenAgeBetweenNo = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and chaid.visit_type=:visitTypeInstance and chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17G"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            ...
                            if (end_date && from_date) {
                                womenAgeBetweenNo2 = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and arrival_time between '" + from_date + "' and '" + end_date + "' and chaid.visit_type=:visitTypeInstance and chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17G"), region: regionListInstance, visitTypeInstance: visitTypeInstance])

                            } else {
                                womenAgeBetweenNo2 = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and chaid.visit_type=:visitTypeInstance and chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17G"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                ...
                <td>
                    ...
                    <table class="table" border="1">
                        ...
                        <%
                            ...
                            if (end_date && from_date) {
                                maleAgeBetweenNo = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and arrival_time between '" + from_date + "' and '" + end_date + "' and chaid.visit_type=:visitTypeInstance and chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17H"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            } else {
                                maleAgeBetweenNo = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and chaid.visit_type=:visitTypeInstance and chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17H"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            }
                            ...
                            if (end_date && from_date) {
                                maleAgeBetweenInstance = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and arrival_time between '" + from_date + "' and '" + end_date + "' and chaid.visit_type=:visitTypeInstance and chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17H"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            } else {
                                maleAgeBetweenInstance = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and chaid.visit_type=:visitTypeInstance and chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17H"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            }
                ...
                <td>
                    ...
                    <table class="table" border="1">
                        ...
                        <%
                            ...
                            if (end_date && from_date) {
                                neonatesInstance = CategoryAvailableChildren.executeQuery("select sum(member_no) from CategoryAvailableChildren where categoryType=:categoryType  and availableMemberHouse.chaid.deleted=false and availableMemberHouse.chaid.distric.region_id=:region and created_at between '" + from_date + "' and '" + end_date + "'  and availableMemberHouse.chaid.visit_type=:visitTypeInstance and availableMemberHouse.chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17D"), region: regionListInstance, visitTypeInstance: visitTypeInstance])

                            } else {
                                neonatesInstance = CategoryAvailableChildren.executeQuery("select sum(member_no) from CategoryAvailableChildren where categoryType=:categoryType  and availableMemberHouse.chaid.deleted=false and availableMemberHouse.chaid.distric.region_id=:region  and availableMemberHouse.chaid.visit_type=:visitTypeInstance and availableMemberHouse.chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17D"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            ...
                            if (end_date && from_date) {
                                neonatesInstance1 = CategoryAvailableChildren.executeQuery("select sum(member_no) from CategoryAvailableChildren where categoryType=:categoryType  and availableMemberHouse.chaid.deleted=false and availableMemberHouse.chaid.distric.region_id=:region and created_at between '" + from_date + "' and '" + end_date + "'  and availableMemberHouse.chaid.visit_type=:visitTypeInstance and availableMemberHouse.chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17D"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                ...
                <td>
                    ...
                    <table class="table" border="1">
                        ...
                        <%
                            ...
                            if (end_date && from_date) {
                                infantsInstance = CategoryAvailableChildren.executeQuery("select sum(member_no)  from CategoryAvailableChildren where categoryType=:categoryType  and availableMemberHouse.chaid.deleted=false and availableMemberHouse.chaid.distric.region_id=:region and created_at between '" + from_date + "' and '" + end_date + "'  and availableMemberHouse.chaid.visit_type=:visitTypeInstance and availableMemberHouse.chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17E"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            } else {
                                infantsInstance = CategoryAvailableChildren.executeQuery("select sum(member_no)  from CategoryAvailableChildren where categoryType=:categoryType  and availableMemberHouse.chaid.deleted=false and availableMemberHouse.chaid.distric.region_id=:region  availableMemberHouse.chaid.visit_type=:visitTypeInstance and availableMemberHouse.chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17E"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            ...
                            if (end_date && from_date) {
                                infantsInstance1 = CategoryAvailableChildren.executeQuery("select sum(member_no)  from CategoryAvailableChildren where categoryType=:categoryType  and availableMemberHouse.chaid.deleted=false and availableMemberHouse.chaid.distric.region_id=:region and created_at between '" + from_date + "' and '" + end_date + "'  and availableMemberHouse.chaid.visit_type=:visitTypeInstance and availableMemberHouse.chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17E"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            } else {
                                infantsInstance1 = CategoryAvailableChildren.executeQuery("select sum(member_no)  from CategoryAvailableChildren where categoryType=:categoryType  and availableMemberHouse.chaid.deleted=false and availableMemberHouse.chaid.distric.region_id=:region  availableMemberHouse.chaid.visit_type=:visitTypeInstance and availableMemberHouse.chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17E"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                ...
                <td>
                    ...
                    <table class="table" border="1">
                        ...
                        <%
                            ...
                            if (end_date && from_date) {
                                pregnantWomenInstance = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and arrival_time between '" + from_date + "' and '" + end_date + "' and  chaid.visit_type=:visitTypeInstance and chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17A"), region: regionListInstance, visitTypeInstance: visitTypeInstance])

                            } else {
                                pregnantWomenInstance = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and chaid.visit_type=:visitTypeInstance and chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17A"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                            ...
                            if (end_date && from_date) {
                                pregnantWomenInstance1 = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and arrival_time between '" + from_date + "' and '" + end_date + "' and  chaid.visit_type=:visitTypeInstance and chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17A"), region: regionListInstance, visitTypeInstance: visitTypeInstance])

                            } else {
                                pregnantWomenInstance1 = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and chaid.visit_type=:visitTypeInstance and chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17A"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                ...
                <td>
                    ...
                    <table class="table" border="1">
                                <%
                                    ...
                                    if (end_date && from_date) {
                                        breastFeedingWomenInstance = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and arrival_time between '" + from_date + "' and '" + end_date + "' and chaid.visit_type=:visitTypeInstance and chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17B"), region: regionListInstance, visitTypeInstance: visitTypeInstance])

                                    } else {
                                        breastFeedingWomenInstance = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and chaid.visit_type=:visitTypeInstance and chaid.repeated=false", [categoryType: DictionaryItem.findByCode("CHAD17B"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                                    }
                                    ...
                                    if (end_date && from_date) {
                                        breastFeedingWomenInstance1 = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and arrival_time between '" + from_date + "' and '" + end_date + "' and chaid.visit_type=:visitTypeInstance and chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17B"), region: regionListInstance, visitTypeInstance: visitTypeInstance])

                                    } else {
                                        breastFeedingWomenInstance1 = AvailableMemberHouse.executeQuery("select sum(member_no)  from AvailableMemberHouse where type_id=:categoryType  and chaid.deleted=false and chaid.distric.region_id=:region and chaid.visit_type=:visitTypeInstance and chaid.repeated=true", [categoryType: DictionaryItem.findByCode("CHAD17B"), region: regionListInstance, visitTypeInstance: visitTypeInstance])
                                    }
